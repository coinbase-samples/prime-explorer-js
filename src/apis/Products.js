...
export const getProducts = async (queryParams) => {
  ...
  const url = `${httpHost}:${port}/api/v1/portfolios/${portfolioId}/products?${queryParams}`;
  const path = `/v1/portfolios/${portfolioId}/products`;

  try {
    const fetchProducts = await makeCall('GET', url, path, '');
